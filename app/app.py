...
class MetaLearningCLI:
    ...
    def show_menu(self) -> None:
        ...
        menu_text.append("1. 🔄 Update Knowledge Base (⚠️  CLEARS ALL DATA)\n", style="cyan")
        ...
        if stats and not stats.get('error'):
            ...
            menu_text.append(f"  • Unique algorithms: {stats.get('unique_algorithms', 0)}\n", style="dim")
            menu_text.append(f"  • Datasets covered: {stats.get('datasets_covered', 0)}\n", style="dim")
    ...
    def inspect_knowledge_base(self) -> None:
        ...
        try:
            ...
            if total_entries == 0:
                self.console.print("📭 No entries found in knowledge base.", style="yellow")
                return
            
            while True:
                # Get entries for current page
                offset = current_page * page_size
                entries = self.repository.get_all(limit=page_size, offset=offset)
                ...
                # Create table
                title = f"Knowledge Base Entries (Page {current_page + 1} of {total_pages}, {total_entries} total)"
    ...
    def _show_entry_details(self, entries: list) -> None:
        ...
        try:
            ...
            if not entry:
                self.console.print(f"❌ Entry with ID {entry_id} not found.", style="red")
                return
            ...
            if entry.meta_vector:
                ...
                if len(entry.meta_vector) <= 5:
                    ...
                    vector_info += f" [{entry.meta_vector[0]:.3f}, {entry.meta_vector[1]:.3f}, ..., {entry.meta_vector[-1]:.3f}]"
                details.add_row("Meta Vector", vector_info)
            else:
                details.add_row("Meta Vector", "Not computed")
            
            details.add_row("Created At", entry.created_at.isoformat() if entry.created_at else "N/A")
            details.add_row("Updated At", entry.updated_at.isoformat() if entry.updated_at else "N/A")
        ...
        while True:
            ...
            try:
                ...
                if choice == 1:
                    ...
                    warning_text = Text()
                    warning_text.append("⚠️  WARNING: Full Knowledge Base Update", style="bold red")
                    warning_text.append("\n\nThis operation will:")
                    ...
                    warning_text.append("\n\nThis process may take several minutes and cannot be undone.", style="yellow")
                    ...
                    # Get confirmation
                    if Confirm.ask("\nDo you want to continue with the full knowledge base update?", default=False):
                        ...
                        self.console.print("❌ Knowledge base update cancelled.", style="yellow")
                elif choice == 2:
                    self.console.print("📊 Inspecting knowledge base...", style="bold magenta")
